...
## Objective
Build a full preview of the Art App using the locked 'Master-Detail Workspace' variation and Sales Intelligence Dashboard design system. Follow the Design Brain prompt exactly, implementing every screen, layout, and component as specified, with all master-detail, modal, and multi-region flows.
...
Variation: Master-Detail Workspace
A decision-centric, master-detail layout with persistent vertical navigation and a right-side detail/inspector panel for deep context and actionable flows.
...
1. App Shell (applies to all pages):
   ...
   - Main content area: split into master (list/table/grid) and detail/inspector (panel, drawer, or modal on mobile)

2. Painting Catalog Listing ("/", all status variants):
   - Layout: Master-Detail Workspace
   - Regions:
     - Master List (left, collapsible on mobile):
       - PaintingTable or PaintingList (rows: image, title, price, size, short description)
       - Filters (above list):
         ...
         - ClearFiltersButton
       - PaginationControls (below list)
     - Detail Panel (right):
       - PaintingDetailCard (shows selected painting: image, title, description, price, size, delivery fee, Buy Now/Add to Cart)
       - RelatedPaintingsList (horizontal scroll)
       - DeliveryInfoTooltip
       - LanguageToggle, ThemeToggle
     - Status Variants:
       - Empty: Master list shows empty state illustration/message, seller sees Add Painting CTA
...
3. Painting Detail Page ("/painting/:id", all status variants):
   - Layout: Master-Detail Workspace (navigated directly or via master list selection)
   - Regions:
     - Master List: PaintingTable (remains visible on desktop, collapses on mobile)
     - Detail Panel:
       - PaintingImageZoom
       - PaintingTitle
       - PaintingDescription
...
4. Checkout Flow ("/checkout", multi-step wizard):
   - Step 1: Delivery Information
     - Single-column form in detail panel; master list shows painting/cart summary
   ...
   - Step 2: Payment Method Selection
     - Master: Painting/cart summary
     - Detail: PaymentMethodList (Cash on Delivery, Bcash, Mobile Wallet, SSL Commerce), icons, tooltips, Continue/Back buttons, DeliverySummaryCard, BuyerSummaryCard, EditDeliveryInfoAction
   ...
   - Step 3: Order Review & Confirmation
     - Master: Painting/cart summary
     - Detail: PaintingSummaryCard, DeliverySummaryCard, PaymentMethodSummaryCard, OrderTotalDisplay, EditSectionAction, ConfirmOrderButton, BackButton, BuyerSummaryCard, EditBuyerInfoAction
     - Status Variants: Default, Empty, Loading, Error, Plan Limit, Permission Denied
   - Step 4: Payment Processing
     - Detail panel: ProcessingSpinner, ProcessingStatusText, TimeoutWarning, RetryButton, BackButton
     - Status Variants: Default, Empty, Loading, Error, Plan Limit, Permission Denied
   - Step 5: Payment Error
     - Detail: PaymentErrorAlert, OrderSummaryCard, BuyerDetailsCard, RetryPaymentButton, ChangePaymentMethodButton
     - Master: PaymentMethodList, SupportContactLink, FAQLink
     - Status Variants: Default, Empty, Loading, Error, Plan Limit, Permission Denied
   - Confirmation ("/checkout/confirmation")
     - Detail: OrderConfirmationBanner, OrderSummaryCard, PaintingDetailsCard, DeliveryDetailsCard, DownloadInvoiceButton, OrderStatusBadge
     - Master: FeedbackPrompt, SupportContactLink, OrderTrackingLink
     - Status Variants: Default, Empty, Loading, Error, Plan Limit, Permission Denied
   - Bangladesh Restriction (invalid address):
     - Detail: BangladeshRestrictionAlert, BuyerDetailsCard, DisabledContinueButton
     - Status Variants: Default, Empty, Loading, Error, Plan Limit, Permission Denied
...
6. Buyer Dashboard ("/dashboard/buyer", all status variants):
   - Layout: Master-Detail Workspace
   - Master List: OrderTable (pending/shipped tabs, each row: painting, delivery, status, actions)
   - Detail Panel: OrderDetailCard (painting, delivery, payment, status, Cancel/Return buttons), NotificationPreferences, SupportContact
...
7. Seller Dashboard ("/dashboard/seller", all status variants):
   - Layout: Master-Detail Workspace
   - Master List: OrderTable (pending/shipped tabs, each row: buyer info, painting, delivery, payment, status, actions), AddNewPaintingButton
   - Detail Panel: OrderDetailCard (buyer info, painting, delivery, payment, status, MarkAsShipped, ReviewRequest), NotificationPreferences, SupportContact
...
8. Settings & Preferences ("/settings"):
   - Detail panel: LanguageToggle, ThemeToggle, NotificationPreferences, AccountInfo, SaveButton
   - Notification Preferences Modal: Email/In-App/SMS toggles

9. Notifications Center ("/notifications", all status variants):
   - Master List: NotificationTable (read/unread, filter by type)
   - Detail Panel: NotificationDetailCard (full message, related actions), EmptyStateIllustration/Message, RetryButton
...
Index URL: https://github.com/ahsanaasim/test-repo-painting-app/blob/ai-build/preview-var2-issue-4-implement-master-detail-workspace-ui-preview-bui/.ops4/design-system/DESIGN_SYSTEM.md
...
## Requirements
- Implement every screen and state listed in the locked Design Brain prompt using the Master-Detail Workspace archetype.
- Use Next.js (App Router), TypeScript, Tailwind CSS, and Shadcn/ui components as specified.
- Apply the full Sales Intelligence Dashboard design system: accent colors, typography, sidebar/topbar shell, and responsive layouts.
- Include all master-detail, modal, and drawer flows for both desktop and mobile breakpoints.
- Populate all fields and filters per the page-by-page data points summary with realistic sample data.
- Implement all status variants (empty, loading, error, permission, plan limit, etc.) for each screen.
...
## Acceptance Criteria
- All screens, layouts, and states from the locked prompt are present and match the specified archetype.
- All fields, filters, and multi-region layouts are implemented with realistic sample data.
- Responsive behavior and accessibility meet the Sales Intelligence Dashboard guidelines.
- Lint passes
- Build passes
...
`ai-build/preview-var2-issue-4-implement-master-detail-workspace-ui-preview-bui`
...
PR title should be:
AI Build: Implement Master-Detail Workspace UI Preview Build
